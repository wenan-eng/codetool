...
export function generateMetadata({params}:{params:{locale:string}}){
  const title = params.locale==='en' ? 'Image Tools' : params.locale==='es' ? 'Herramientas de Imagen' : '图片处理工具'
  ...
  return buildPageMetadata({ locale: params.locale, path: "image", title, description: desc })
}
export default function ImageCategory({params}:{params:{locale:string}}){
  const locale = params.locale
  const filtered = (tools as any[]).filter(t=>t.category==="图片处理")
  const title = locale==='en' ? 'Images' : locale==='es' ? 'Imágenes' : '图片处理'
  return (
    <div>
