...
export function generateMetadata({params}:{params:{locale:string}}){
  const title = params.locale==='en' ? 'Media Tools' : params.locale==='es' ? 'Herramientas de Medios' : '影音媒体工具'
  ...
  return buildPageMetadata({ locale: params.locale, path: "media", title, description: desc })
}
export default function ImageCategory({params}:{params:{locale:string}}){
  const locale = params.locale
  const filtered = (tools as any[]).filter(t=>t.category==="影音媒体")
  const title = locale==='en' ? 'Media' : locale==='es' ? 'Medios' : '影音媒体'
  return (
    <div>
