...
import { transformUtf8 } from "@/lib/utf8Codec"
...
const SAMPLES: Record<string, string> = {
  zh: `你好世界，Hello World！\n{"name":"张三","city":"北京"}\n😀 Emoji 与中文混排内容`,
  en: `你好世界，Hello World！\n{"name":"Zhang San","city":"Beijing"}\n😀 Emoji and Chinese mixed content`,
  es: `你好世界，Hola Mundo！\n{"name":"Zhang San","city":"Pekín"}\n😀 Emoji y contenido mixto en chino`,
}

export default function Utf8Editor({ locale = "zh" }: { locale?: string }) {
  const toolMsgs = (messagesMap[locale] || zh).utf8Codec
  const editorFallback = (messagesMap[locale] || zh).editor
  const msgs = {
    encode: toolMsgs?.encode || editorFallback?.encode || "UTF8编码",
    decode: toolMsgs?.decode || editorFallback?.decode || "UTF8解码",
    ...
    inputPlaceholder: toolMsgs?.inputPlaceholder || "请输入要进行 UTF-8 与中文转换的文本内容...",
  ...
  const handleEncode = () => {
    setOutput(transformUtf8(input, "encode"))
  }
  const handleDecode = () => {
    setOutput(transformUtf8(input, "decode"))
  }
  const handleSample = () => {
