...
import { flatten } from "@/lib/jsonFlatten"
...
const SAMPLES: Record<string, string> = {
  zh: JSON.stringify({ a: { b: 1, c: [2, 3, { d: "hi" }] }, e: 4 }, null, 2),
  en: JSON.stringify({ a: { b: 1, c: [2, 3, { d: "hi" }] }, e: 4 }, null, 2),
  es: JSON.stringify({ a: { b: 1, c: [2, 3, { d: "hi" }] }, e: 4 }, null, 2),
}

export default function JsonFlattenEditor({ locale = "zh" }: { locale?: string }) {
  const dict = (messagesMap[locale] || zh) as any
  const jf = dict.jsonFlatten || {}
  const editorFallback = dict.editor || {}
  const msgs = {
    flatten: jf.flatten || (locale === "en" ? "Flatten" : locale === "es" ? "Aplanar" : "扁平化"),
    ...
    inputPlaceholder: jf.inputPlaceholder || (locale === "en" ? "Paste nested JSON, e.g. {\"a\":{\"b\":1}}" : locale === "es" ? "Pegue JSON anidado, ej. {\"a\":{\"b\":1}}" : '请输入嵌套 JSON，例如 {"a":{"b":1}}'),
    outputPlaceholder: jf.outputPlaceholder || (locale === "en" ? "Flattened result will appear here" : locale === "es" ? "El resultado aplanado aparecerá aquí" : "扁平化结果将显示在这里"),
    localNote: jf.localNote || editorFallback.localNote || "所有操作均在浏览器本地完成，不上传数据",
    inputLabel: jf.inputLabel || (locale === "en" ? "Nested JSON" : locale === "es" ? "JSON Anidado" : "嵌套 JSON"),
    outputLabel: jf.outputLabel || (locale === "en" ? "Flattened (dot notation)" : locale === "es" ? "Aplanado (notación punto)" : "扁平结果（点号）"),
  }
    ...
    <div className="space-y-4">
      <div className="bg-white rounded-xl border shadow-sm overflow-hidden">
        <div className="flex flex-wrap gap-2 p-3 border-b bg-gray-50">
          <button onClick={handleFlatten} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700">{msgs.flatten}</button>
