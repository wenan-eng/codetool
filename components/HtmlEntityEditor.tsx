...
import { transformHtmlEntity } from "@/lib/htmlEntityCodec"
...
const SAMPLES: Record<string, string> = {
  zh: `<div class="box">你好 & 世界</div>\n<a href="/search?q=中文">链接 "引号" 与 '单引号'</a>\n© ™ — 😀 特殊符号`,
  en: `<div class="box">Hello & World</div>\n<a href="/search?q=test">link "quotes" and 'apostrophes'</a>\n© ™ — 😀 special symbols`,
  es: `<div class="box">Hola y Mundo</div>\n<a href="/search?q=prueba">enlace "comillas" y "apóstrofes"</a>\n© ™ — 😀 símbolos especiales`,
}

export default function HtmlEntityEditor({ locale = "zh" }: { locale?: string }) {
  const toolMsgs = (messagesMap[locale] || zh).htmlEntityCodec
  const editorFallback = (messagesMap[locale] || zh).editor
  const msgs = {
    encode: toolMsgs?.encode || editorFallback?.encode || "HTML编码",
    ...
    inputPlaceholder: toolMsgs?.inputPlaceholder || "请输入要进行 HTML 实体编码/解码的文本内容...",
  ...
  const handleEncode = () => {
    setOutput(transformHtmlEntity(input, "encode"))
  }
  const handleDecode = () => {
    setOutput(transformHtmlEntity(input, "decode"))
  }
  const handleSample = () => {
